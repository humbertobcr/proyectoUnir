            ...
            with open(ruta_archivo, 'w', encoding='utf-8') as f:
                for _, row in df_total.iterrows():
                    ...
                    document = {
                        ...
                        "rendimiento_aritmetico": float(row["rendimiento_aritmetico"]) if pd.notna(
                            row["rendimiento_aritmetico"]) else None,
                        "rendimiento_logaritmico": float(row["rendimiento_logaritmico"]) if pd.notna(
                            row["rendimiento_logaritmico"]) else None,
                        "rendimiento_acumulado_arit": float(row["rendimiento_acumulado_arit"]) if pd.notna(
                            row["rendimiento_acumulado_arit"]) else None,
                        "rendimiento_acumulado_log": float(row["rendimiento_acumulado_log"]) if pd.notna(
                            row["rendimiento_acumulado_log"]) else None,
                        "volatilidad_005d": float(row["volatilidad_005d"]) if pd.notna(
                            row["volatilidad_005d"]) else None,
                        "volatilidad_010d": float(row["volatilidad_010d"]) if pd.notna(
                            row["volatilidad_010d"]) else None,
                        "volatilidad_015d": float(row["volatilidad_015d"]) if pd.notna(
                            row["volatilidad_015d"]) else None,
                        "volatilidad_020d": float(row["volatilidad_020d"]) if pd.notna(
                            row["volatilidad_020d"]) else None,
                        "volatilidad_030d": float(row["volatilidad_030d"]) if pd.notna(
                            row["volatilidad_030d"]) else None,
                        "volatilidad_060d": float(row["volatilidad_060d"]) if pd.notna(
                            row["volatilidad_060d"]) else None,
                        "volatilidad_090d": float(row["volatilidad_090d"]) if pd.notna(
                            row["volatilidad_090d"]) else None,
                        "volatilidad_252d": float(row["volatilidad_252d"]) if pd.notna(
                            row["volatilidad_252d"]) else None,
